...
public class UserWebSocketStarter extends WebSocketAdapter implements WebSocketStarter {
  ...
  @Resource private UserMessageListener userMessageListener;
  @Resource private DiscordHelper discordHelper;
  ...
  public synchronized void start() throws Exception {
    ...
    this.socket =
        webSocketFactory.createSocket(
            this.discordHelper.getWss() + "/?encoding=json&v=9&compress=zlib-stream");
  ...
  public void onBinaryMessage(WebSocket websocket, byte[] binary) throws Exception {
    ...
    if (opCode == WebSocketCode.HELLO) {
      if (this.heartbeatTask == null && this.heartExecutor != null) {
        ...
        this.heartbeatTask =
            this.heartExecutor.scheduleAtFixedRate(
  ...
  public void onDisconnected(
    ...
    if (code >= 4010 || code == 4004) {
      log.warn(
          "[gateway] Websocket closed and can't reconnect! code: {}, reason: {}",
          code,
          closeReason);
      System.exit(code);
      return;
    ...
    log.warn(
        "[gateway] Websocket closed and will be reconnect... code: {}, reason: {}",
        code,
        closeReason);
    ThreadUtil.execute(
        () -> {
  ...
  private void retryStart(int currentRetryTime) throws Exception {
    try {
      ...
      if (currentRetryTime < CONNECT_RETRY_LIMIT) {
        currentRetryTime++;
        log.warn(
            "[gateway] Websocket start fail, retry {} time... error: {}",
            currentRetryTime,
            e.getMessage());
        Thread.sleep(5000L);
  ...
  private DataObject createAuthData() {
    ...
    DataObject connectionProperties =
        DataObject.empty()
