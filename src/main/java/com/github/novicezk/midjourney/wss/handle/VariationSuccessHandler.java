...
/**
 * variation消息处理. 完成(create): **cat** - Variations (Strong或Subtle) by <@1012983546824114217>
 * (relaxed) 完成(create): **cat** - Variations by <@1012983546824114217> (relaxed)
 */
@Component
public class VariationSuccessHandler extends MessageHandler {
  private static final String CONTENT_REGEX_1 =
      "\\*\\*(.*?)\\*\\* - Variations by <@\\d+> \\((.*?)\\)";
  private static final String CONTENT_REGEX_2 =
      "\\*\\*(.*?)\\*\\* - Variations \\(.*?\\) by <@\\d+> \\((.*?)\\)";
  ...
  public void handle(MessageType messageType, DataObject message) {
    ...
    if (MessageType.CREATE.equals(messageType) && parseData != null && hasImage(message)) {
      // 这里SET中搜索两次两次。目的是为了VARY类型的任务也可以置为成功。
      TaskCondition condition =
          new TaskCondition()
              .setActionSet(Set.of(TaskAction.VARIATION, TaskAction.VARY))
