...
public class DiscordServiceImpl implements DiscordService {
  ...
  void init() {
    ...
    this.discordUploadUrl =
        serverUrl + "/api/v9/channels/" + this.discordChannelId + "/attachments";
    this.discordSendMessageUrl =
        serverUrl + "/api/v9/channels/" + this.discordChannelId + "/messages";
  ...
  @Override
  public Message<Void> upscale(
      String messageId, int index, String messageHash, int messageFlags, String nonce) {
    String paramsStr =
        replaceInteractionParams(this.upscaleParamsJson, nonce)
  ...
  @Override
  public Message<Void> variation(
      String messageId, int index, String messageHash, int messageFlags, String nonce) {
    String paramsStr =
        replaceInteractionParams(this.variationParamsJson, nonce)
            .replace("$message_id", messageId)
            .replace("$index", String.valueOf(index))
            .replace("$message_hash", messageHash);
    paramsStr = new JSONObject(paramsStr).put("message_flags", messageFlags).toString();
  ...
  @Override
  public Message<Void> reroll(
      String messageId, String messageHash, int messageFlags, String nonce) {
    String paramsStr =
        replaceInteractionParams(this.rerollParamsJson, nonce)
            .replace("$message_id", messageId)
            .replace("$message_hash", messageHash);
    paramsStr = new JSONObject(paramsStr).put("message_flags", messageFlags).toString();
    return postJsonAndCheckStatus(paramsStr);
  ...
  public Message<Void> describe(String finalFileName, String nonce) {
    ...
    String paramsStr =
        replaceInteractionParams(this.describeParamsJson, nonce)
  ...
  @Override
  public Message<Void> blend(
      List<String> finalFileNames, BlendDimensions dimensions, String nonce) {
    ...
    for (int i = 0; i < finalFileNames.size(); i++) {
      ...
      JSONObject option =
          new JSONObject().put("type", 11).put("name", "image" + (i + 1)).put("value", i);
      options.put(option);
    }
    options.put(
        new JSONObject()
  ...
  private String replaceInteractionParams(String paramsStr, String nonce) {
    return paramsStr
        .replace("$guild_id", this.discordGuildId)
  ...
  public Message<Void> zoom(
      String messageId, int index, String messageHash, int messageFlags, String nonce) {
    String degree = index == 1 ? "75" : "50";
    String paramsStr =
        replaceInteractionParams(this.zoomParamsJson, nonce)
            .replace("$message_id", messageId)
  ...
  @Override
  public Message<Void> vary(
      String messageId, int index, String messageHash, int messageFlags, String nonce) {
    String degree = index == 1 ? "low_variation" : "high_variation";
    String paramsStr =
        replaceInteractionParams(this.varyParamsJson, nonce)
            .replace("$message_id", messageId)
            .replace("$degree", degree)
            .replace("$message_hash", messageHash);
    paramsStr = new JSONObject(paramsStr).put("message_flags", messageFlags).toString();
